...
fn get_available_operations(path: &str) -> HashMap<String, Option<u16>> {
    ...
    */
    ...
    let mut root: HashMap<_, _> = json.try_into().expect("expected JSON object at the root");
    let op_rows: HashMap<_, _> = root
        .remove("operations")
        .expect("expected 'operations' key")
        .try_into()
        .expect("expected JSON object as 'operations' value");

    for (_, row_values) in op_rows {
        let row: HashMap<_, _> = row_values
            .try_into()
            .expect("expected JSON object as row value");
        for (_, col_values) in row {
            let col: Vec<_> = col_values
                .try_into()
                .expect("expected JSON array as column value");
            for op_value in col {
                let op: String = op_value
                    .try_into()
                    .expect("expected JSON string as operator value");
                // println!("Operation {}", s);
...
// gh512
#[derive(Clone)]
pub(crate) struct GreedyBanCost {
...
impl GreedyBanCost {
        ...
        P: AsRef<str>,
    {
        ...
        let ban_cost = GreedyBanCost::from_available(operations.keys());
...
impl CostFunction<SymbolLang> for GreedyBanCost {
    type Cost = usize;
        ...
        C: FnMut(Id) -> Self::Cost,
    {
        ...
        let op_cost =
            if symbol.as_str().starts_with("Dummy") || symbol.as_str().starts_with("const") {
                ...
                10_000 // does not seem to like f64::INFINITY
            };
        enode.fold(op_cost, |sum, id| sum + costs(id))
    }
}

impl LpCostFunction<SymbolLang, ()> for GreedyBanCost {
    // NOTE: this will NOT be used during MCTS search; only to satisfy mcts-geb's trait bound
    fn node_cost(
        &mut self,
...
impl LookupCost {
        ...
        P: AsRef<str>,
    {
        ...
        let costs = operations
            ...
            .map(|(s, c)| {
                let cost =
                    (total_ops - (c.map(|n| n as f64).unwrap_or(total_ops - 1.0))) / total_ops;
                (Symbol::from(s), cost)
            })
